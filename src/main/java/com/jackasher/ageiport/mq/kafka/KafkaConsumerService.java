...
public class KafkaConsumerService {
    ...
    public void handleAttachmentTask(@Payload ProcessContext<IrMessageData, IrMessageQuery> message,
                                     @Header(KafkaHeaders.RECEIVED_PARTITION_ID) int partition,
                                     @Header(KafkaHeaders.OFFSET) long offset,
                                     Acknowledgment ack) {
        try {
            log.info("接收到附件处理任务，SubTaskID: {}, Partition: {}, Offset: {}, 消息数量: {}",
                    message.subTaskId, partition, offset,
            ...
            log.info("附件处理任务完成，SubTaskID: {}", message.subTaskId);

        } catch (Exception e) {
            log.error("处理附件任务失败，SubTaskID: {}, Partition: {}, Offset: {}",
                    message.subTaskId, partition, offset, e);
            ...
            if (shouldAcknowledgeOnError(e)) {
                ack.acknowledge();
                log.warn("虽然处理失败，但已确认消息以避免重复处理，SubTaskID: {}", message.subTaskId);
            } else {
                log.warn("处理失败，消息将保持未确认状态以便重试，SubTaskID: {}", message.subTaskId);
            }
        }
    }
     ...
     * @param message 附件任务消息
     */
    private void processAttachmentTask(ProcessContext<IrMessageData, IrMessageQuery> message) {
        try {
            String subTaskId = message.subTaskId;

            if (batchDataProcessingService != null && message.messages != null && !message.messages.isEmpty()) {
                log.info("开始处理附件，SubTaskID: {}, 文件数量: {}",
                        message.subTaskId, message.messages.size());
                ...
                log.info("附件处理完成，SubTaskID: {}", message.subTaskId);
            } else {
                log.warn("无法获取附件处理服务或消息为空，SubTaskID: {}", message.subTaskId);
            }

        } catch (Exception e) {
            log.error("处理附件时发生异常，SubTaskID: {}", message.subTaskId, e);
