...
//import com.jackasher.ageiport.service.data_processing_service.impl.AttachmentProcessingServiceImpl;
...
//
/**
 * 已废弃,GenericProcessingDispatcher抽象成通用的分发器作为替代,可做参考
 * 批次数据处理工具类 - 简化附件处理逻辑
 * 提供不同策略处理导出批次数据
 * @author jackasher
 */
//@Deprecated
//public class BatchProcessingDispatcher {
//
//    private static final Logger log = LoggerFactory.getLogger(BatchProcessingDispatcher.class);
//
...
//    private static final Map<BatchDataProcessMode, ProcessHandler> handlers = new EnumMap<>(BatchDataProcessMode.class);
//
//    static {
//        // 初始化处理器映射
//        handlers.put(BatchDataProcessMode.SYNC, BatchProcessingDispatcher::processSyncMode);
//        handlers.put(BatchDataProcessMode.ASYNC, BatchProcessingDispatcher::processAsyncMode);
//        handlers.put(BatchDataProcessMode.DEFERRED, BatchProcessingDispatcher::processDeferredMode);
//        handlers.put(BatchDataProcessMode.NONE, BatchProcessingDispatcher::processNoneMode);
//        handlers.put(BatchDataProcessMode.RABBITMQ, BatchProcessingDispatcher::processMqMode);
//        handlers.put(BatchDataProcessMode.KAFKA, BatchProcessingDispatcher::processKafkaMode);
//    }
//
//    @FunctionalInterface
...
//     */
//    public static void processBatchData(List<IrMessageData> messages, String subTaskId,
...
//        // =======================================================
//
//        BatchDataProcessMode batchDataProcessMode = IrMessageUtils.getResolvedParams(query).getBatchDataProcessMode();
////        BatchDataProcessMode mode = parseMode(batchDataProcessMode);
//
//        log.info("子任务 {} 使用附件处理模式: {}", subTaskId, batchDataProcessMode);
//
//        // 使用映射表 + 函数式编程替代switch
//        ProcessContext context = new ProcessContext(messages, subTaskId, subTaskNo, query, mainTaskId);
//        handlers.getOrDefault(batchDataProcessMode, BatchProcessingDispatcher::processNoneMode).handle(context);
//    }
//
//
//    // ==================== 延迟队列的处理策略 ====================
...
//    }
//
//    private static BatchDataProcessMode parseMode(String modeStr) {
//        if (modeStr == null || modeStr.trim().isEmpty()) {
//            return BatchDataProcessMode.ASYNC; // 默认异步
//        }
//
//        try {
//            return BatchDataProcessMode.valueOf(modeStr.toUpperCase());
//        } catch (IllegalArgumentException e) {
//            log.warn("无效的附件处理模式: {}, 使用默认的ASYNC模式", modeStr);
//            return BatchDataProcessMode.ASYNC;
//        }
//    }
//
//    // ==================== 具体处理策略实现 ====================
...
//            AttachmentProcessingServiceImpl service = SpringContextUtil.getBean(
//                "attachmentProcessingServiceImpl", AttachmentProcessingServiceImpl.class);
//            service.processData(ctx.messages, ctx.subTaskId, ctx.subTaskNo, null);
...
//
//            // 2. 创建消息体
//            AttachmentTaskMessage message = new ProcessContext(
//                    ctx.messages,
//                    ctx.subTaskId,
//                    ctx.subTaskNo,
//                    null,
...
//            AttachmentProcessingServiceImpl service = SpringContextUtil.getBean(
//                "attachmentProcessingServiceImpl", AttachmentProcessingServiceImpl.class);
//            service.processDataAsync(ctx.messages, ctx.subTaskId, ctx.subTaskNo, null)
//                .whenComplete((result, throwable) -> {
//                    if (throwable != null) {
//                        log.error("【异步模式】子任务 {} 的附件处理失败: {}", ctx.subTaskId, null, throwable);
//                    } else {
