...
public class MqProducerService {
    ...
    public void sendAttachmentTask(AttachmentTaskMessage message) {
        try {
            log.info("准备发送附件处理任务到MQ，SubTaskID: {}", message.getSubTaskId());
            rabbitTemplate.convertAndSend(ATTACHMENT_EXCHANGE, ATTACHMENT_ROUTING_KEY, message);
            log.info("成功发送附件处理任务到MQ，SubTaskID: {}", message.getSubTaskId());
        } catch (Exception e) {
            log.error("发送附件处理任务到MQ失败, SubTaskID: {}", message.getSubTaskId(), e);
